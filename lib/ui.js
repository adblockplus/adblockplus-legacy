...
let UI = exports.UI =
{
  ...
  restoreToolbarIcon: function(/**Element*/ toolbox, /**Element*/ icon)
  {
    ...
    if (position == null)
    {
      ...
      // attribute somewhere.
      let toolbars = toolbox.externalToolbars.concat(toolbox.toolbarset.children);
  ...
  showToolbarIcon: function(/**Element*/ toolbox, /**String*/ position)
  {
    ...
    if (visible != "visible")
    {
      ...
      if (icon && icon.parentNode.localName == "toolbar")
      {
        let items = icon.parentNode.currentSet.split(",");
        let index = items.indexOf("abp-toolbarbutton");
        if (index >= 0)
        {
          items.splice(index, 1);
          icon.parentNode.currentSet = items.join(",");
          changed = true;
        }
      }
    }
    else if (parent)
    {
      ...
      if (index >= 0)
        items.splice(index, 0, "abp-toolbarbutton");
      else
        items.push("abp-toolbarbutton");
      parent.currentSet = items.join(",");
      changed = true;
    }

    // Notify browser UI and our own listeners about the change
    if ("customizeDone" in toolbox)
      toolbox.customizeDone(changed);

    let event = toolbox.ownerDocument.createEvent("Events");
    event.initEvent("aftercustomization", true, true);

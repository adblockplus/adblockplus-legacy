...
Cu.reportError(5);
  ...
  {
    Cu.reportError("flags");
    let flags = ["URI_SAFE_FOR_UNTRUSTED_CONTENT", "HIDE_FROM_ABOUTABOUT", "URI_CAN_LOAD_IN_CHILD"];
    let result = 0;
    for (let flag of flags)
      if (flag in Ci.nsIAboutModule)
        result = result | Ci.nsIAboutModule[flag];
  ...
  {
    Cu.reportError("channel: " + [loadInfo.loadingPrincipal.URI.spec, loadInfo.securityFlags, loadInfo.contentPolicyType]);

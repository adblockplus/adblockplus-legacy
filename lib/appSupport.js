...
exports.removeBrowserClickListeners = function removeBrowserClickListeners(/**Window*/ window)
{
  ...
  if (browser)
  {
    ...
    for (let i = 0; i < listeners.length; i++)
      browser.removeEventListener("click", listeners[i], false);
  }
...
switch (application)
{
  ...
  case "fennec2":
  {
    ...
    UI.updateIconState = function fmn_updateIconState(window, icon)
    {
      ...
      let icon = require("info").addonRoot + "icon64.png";
      menuItem = window.NativeWindow.menu.add(actionText, icon, toggleWhitelist.bind(null, window));
    };
